...
function carregada() {
  ...
  function restoreList() {
    const savedList = JSON.parse(window.localStorage.TaskStore).length;
    for (let index = 0; index < savedList; index += 1) {
      const newLi = document.createElement("li");
      let text = JSON.parse(window.localStorage.TaskStore)[index];
      let name = JSON.parse(window.localStorage.TaskClassStore)[index];
      newLi.innerText = text;
      newLi.className = name;
      newLi.addEventListener('click', selectedItem);
      newLi.addEventListener('dblclick', completedTask);
      list.appendChild(newLi);
    }
  };
...
// link : https://developer.mozilla.org/pt-BR/docs/Web/API/Node/insertBefore
  function goUp() {
    ...
    return list.insertBefore(selected, selected.previousElementSibling);
  }
  up.addEventListener("click", goUp);
  
  function goDown() {
    const selected = document.querySelector('.selected');
    return list.insertBefore(selected, selected.nextElementSibling.nextElementSibling);
  }
  down.addEventListener("click", goDown);
}
window.onload = carregada;